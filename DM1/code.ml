...
let rec mots_of_trie = function
  ...
  | N(c, g, d) ->
      g
      |> mots_of_trie
      |> List.map (List.cons c) (* Pour chaque mot de l'ensemble de mots à gauche, on ajoute le caractère au début *)
...
let trie_of_list_tr str_list =
  let rec aux trie = function
    | [] -> trie
    | hd::tl ->
      aux (hd |> mot_of_string |> insere trie) tl
  in
  aux V str_list

(* Question 5 - sans se prendre la tête TODO: Garder ? *)
(* let trie_of_list =
  List.fold_left
    (fun trie str ->
      str
...
let test_exercice2 () =
  ...
  (* Test question 10 *)
  let tab1 = tableau_occurences "aaaaaaa" in
  assert (tab1.(0) = 7);
  assert (tab1
          |> Array.to_list
          |> List.drop 1
  ...
  let tab2 = tableau_occurences "abbccc" in
  assert (tab2.(0) = 1);
  assert (tab2.(1) = 2);
  assert (tab2.(2) = 3);
  assert (tab2
...
(* Question 1 *)
(* Vérifie si occ_sous_mot est le tableau d'occurence d'un sous mot
   du mot dont occ_mot est le tableau d'occurence *)
...
let tableau_occurences_mot mot =
  let tab = Array.make 26 0 in
...
let sous_mots trie mot_str =
  let occurences = tableau_occurences mot_str in
  trie |> iter_trie (fun mot ->
    let est_sous_mot =
      mot
      |> tableau_occurences_mot
      |> est_sous_mot occurences
    in
...
(* Question 2 *)
(* TODO: Remove let afficher_anagrammes trie mot_str =
  let occurences = tableau_occurences mot_str in

  let rec aux trie mot_courant =
    match trie with
    | V -> ()
    | N('$', V, d) ->
      if mot_courant
         |> tableau_occurences_mot
         |> (=) occurences then begin
        mot_courant |> List.rev |> afficher_mot;
        print_newline ()
      end;
      aux d mot_courant
    | N(c, g, d) ->
...
let affiche_decomposition trie mot_str =
  ()
  (* mot_str
  |> filtrer_sous_mots trie
  |> iter_trie (fun sous_mot ->
    let sous_mot_occ = sous_mot |> tableau_occurences_mot in

  ) *)
  (* let sous_mot'_occ = sous_mot' |> tableau_occurences_mot in
  let occ_diff = Array.map2 (+) sous_mot_occ sous_mot'_occ in
  if occ_diff = occ then begin
    print_string "\"";
    afficher_mot sous_mot;
    print_string " ";
    afficher_mot sous_mot';
    print_string "\"\n";
  end *)
...
let test_exercice4 () =
  ...
  (* Test question 5 *)
  affiche_decomposition ods6_lowercase "barrage"
